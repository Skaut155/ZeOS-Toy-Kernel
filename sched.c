...
// Waiting for tick queue
struct list_head waitingtick;
...
void init_idle (void)
{
  ...
  c->PID=0;
  c->TID=0;  /* IDLE's thread will have TID = 0 */
...
void init_task1(void)
{
  ...
  c->PID=1;
  c->TID=1; /* The first thread of the INIT process will have TID = 1 */
  c->user_stack_pages=1;
  ...
  set_user_pages(c);
  /* We map a physical page to the page corresponding to the initial user_esp, that will
  dynamically grow */
  int pag_fisica = alloc_frame();
  set_ss_pag(get_PT(c), PAG_LOG_INIT_DATA+NUM_PAG_DATA+3, pag_fisica);
  
  // We set a page for the TLS (currently just errno)
  int pf = alloc_frame();
  set_ss_pag(get_PT(c), 1022, pf);
  c->errno_pf = pf;
...
void inner_task_switch(union task_union *new)
{
  ...
  set_ss_pag(get_PT(&(new->task)), 1022, new->task.errno_pf);
  
  /* TLB flush. New address space */
  if (new->task.PID != current()->PID) set_cr3(new_DIR); // If the new thread is part of the same process, there is no need to update cr3
